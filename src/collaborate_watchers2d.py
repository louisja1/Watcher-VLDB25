...
class RoutineBaseline2D:
    ...
    def post_query(self, rs1, rs2, true_card):
        start_time_2 = time.time()
...
class TwoTierWatcher1D:
    def __init__(
        ...
        cursor=None,
        n_tabs_in_from=None,
        query_pool_size=1500, # this is the size of query_pool.
        training_size=1500,
        triggers=None,
        random_seed=None,
...
class JoinCardCorrector:
    ...
    def _compute_overlap_ratio(self, rs1, rs2):
        return (max(0, min(rs1.ub, rs2.ub) - max(rs1.lb, rs2.lb))) / (max(rs1.ub, rs2.ub) - min(rs1.lb, rs2.lb))
    def search(self, rss):
        assert len(rss) == self.n_tabs
        _key = tuple(rss)
...
def load_trainingset(
    ...
    with open(trainingset_filename, "r") as fin:
        ...
        if deployment == "routinely":
            ...
            for lb1, ub1, lb2, ub2, card in two_way_training_set:
                rs1 = RangeSelection(tabs_with_sel[0], lb1, ub1)
                rs2 = RangeSelection(tabs_with_sel[1], lb2, ub2)
                deployments[two_way_model_name].insert_to_query_pool(rs1, rs2, card)
        ...
        for row_num, line in enumerate(tqdm(fin)):
            ...
            if op_type in ["I", "D"]:
                ...
                if deployment in ["routinely", "watcher", "twotier"]:
                    ...
                    if two_way_model_name is not None:
                        assert len(tab_to_pre_query_info[two_way_model_name]["rs"]) == 2
                    ...
                    if deployment == "twotier" and enable_correct_the_joins:
                        ...
                        _, single_only_join_order, single_only_scan_mtd = inject_multiple_single_table_sel_and_get_cost(
                            pointers_and_sels=[(single_tab_to_qid[tab], tab_to_pre_query_info[tab]["model_pred"] / deployments[tab].cur_table_size) for tab in tabs_with_sel],
                        ...
                        if not flag_for_correcting_join:
                            ...
                            plan_cost = inject_multiple_single_table_sel_and_get_cost(
                                pointers_and_sels=[(single_tab_to_qid[tab], tab_to_pre_query_info[tab]["true_card"] / deployments[tab].cur_table_size) for tab in tabs_with_sel],
                                n_tabs_in_from=n_tabs_in_from,
                                q=cur_sql,
                                hint=hint,
                                plan_info=False,
                            ...
                            _, join_order, scan_mtd = inject_single_and_join_sels_and_get_cost(
                                pointers=[single_tab_to_qid[tab] for tab in tabs_with_sel] + _join_qids,
                            ...
                            plan_cost = inject_single_and_join_sels_and_get_cost(
                                pointers=[single_tab_to_qid[tab] for tab in tabs_with_sel] + [tableset_to_joinid[two_way_tableset]],
    ...
    with open(json_filename, "r") as json_fin:
        ...
        if deployment == "routinely":
            deployments[two_way_model_name] = RoutineBaseline2D(
                ...
                cursor=cursor,
                query_pool_size=1500,
                counting_sql=counting_sql[two_way_model_name],
                random_seed=2023,
                based_on=update_based_on[two_way_model_name],
                X=update_after[two_way_model_name],
            )
        elif deployment == "watcher":
        ...
        if deployment == "twotier":
            for tab in tabs_with_sel:
                deployments[tab].setup_single_table_pointer(single_tab_to_qid[tab])
