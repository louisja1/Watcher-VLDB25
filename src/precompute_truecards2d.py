    ...
    with open(trainingset_filename, "r") as fin, open(new_trainingset_filename, "w") as fout:
        ...
        for i in tqdm(range(len(lines))):
            ...
            for j in range(len(row)):
                if row[j] in ["true_card1", "true_card2", "true_join_card12"]:
                    ...
                    if "2" in row[j]:
                        counting_sql = counting_sql.replace("{lb2}", str(bounds[1][0])).replace("{ub2}", str(bounds[1][1]))
